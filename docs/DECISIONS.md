...
**Scope amended by D20.** The single-`k` form remains the complete aperiodic model for every
state except N3. N3 now sums two independently realized components with the same exponent and
different knees. The registered 45 Hz claim remains unmodelled; D20's second source knee is an
8 Hz value fitted independently against HMC and must not be presented as that claim.
...
## D20 — N3 uses two aperiodic timescales, not canonical-band compensation

**Decided.** N3's aperiodic PSD is the variance-preserving sum

`P_N3(f) = (1-q) P(f; chi_n3, k_slow) + q P(f; chi_n3, k_fast)`.

The components are independent realizations through the same lead-field modes. They share
`chi_n3`; only their correlation timescales differ. Fitted against the HMC four-derivation
pipeline, `q = 0.20` and the fast source knee is 8 Hz.

**Why this form.** The first attempted repair retained an N2-like component. It added the missing
power, but the amount required pushed the recovered N3 exponent below the HMC distribution and
never produced a physical fitted knee. Separate theta, alpha, sigma and beta gains would fit the
same PSD with four counterbalancing controls and no generative interpretation. The two-timescale
model uses two joint parameters, retains the already fitted N3 slope, and creates the curvature
and broadband tail in one mechanism.

**Circularity boundary.** The fit objective contains HMC RMS and five broad spectral fractions.
The independently measured HMC exponent and knee are guards, not additional weighted copies of
the same PSD. AASM thresholds and generated event labels are absent from both selection and
held-out validation.

**Truth consequence.** A single `chi`/`knee` pair no longer completely describes an exported N3
record. Epoch schema v2 adds `aperiodicComponents`, including each component's variance share;
the v1 primary fields remain for compatibility.

---

## D21 — respiration is a resumable controller; a display chunk is not a breath epoch

**Decision.** Respiration is generated by one serializable breath-to-breath controller carrying
the subject rate phenotype, current breath, sample offset, fast and slow timing states, depth,
and complete RNG state. Whole-record export is a wrapper around that same state transition. The
live stream advances the controller when it prepares a display chunk instead of calling an
independent respiratory generator for each chunk.

**Why this form.** Independent lognormal periods could make the carrier imperfectly periodic,
but could not make respiratory dynamics physiological: every state had the same CV, zero serial
memory, constant depth, negligible infraslow power, and a phase reset every 90 seconds. Adding
independent jitter for depth, duty cycle, pauses and rate would improve appearance while retaining
the same structural defect. Two correlated latent timescales keep mean, marginal variability and
memory separately inspectable, and let R2's cardiac model consume the same state without turning
ECG parameters into compensatory respiratory controls.

**Subject before state.** One named RNG substream gives the seed a stable rate multiplier. The
literature stage mean is then an offset on that phenotype; state changes do not create a new
person. Gutierrez et al.'s group means replace the invented broad intervals, including a separate
N1 row. Cross-sectional rate spread is retained only as an explicitly provisional approximation
to a subject effect.

**Morphology boundary.** Pause probabilities and mean duration match the BreathMetrics/NeuroKit2
reference implementation as feature anchors, not as universal normal-physiology claims. Timing,
depth, I:E variability and latent variance allocation remain provisional until fitted to an
external belt corpus. Generator version 0.6.0 records the output change.

**Composition correction.** The mechanical movement artifact is added after the stochastic and
state-specific aperiodic background gain. A mechanical electrode artifact no longer inherits
N3's 1.9× cortical-background gain. This is a causal-layer correction, not an amplitude fit.

**Deferred boundary.** Cardiac beat scheduling still restarts at a display join. R1 fixes the
respiratory phase and belt exactly; the remaining RR join error is R2's acceptance failure and is
not hidden by calling the respiratory controller complete.

---

## D22 — cardiac timing lives in RR space and shares the respiratory state

**Decision.** ECG beat times are generated by a serializable RR-domain controller. For beat
`i`, the interval is

`RR_i = meanRR_subject,state + A_RSA,state * depth_i * sin(phi_i + offset) + h_fast,i + h_slow,i`,

where the two non-respiratory terms are correlated latent processes. Their combined variance is
chosen in quadrature so the requested stage SDNN is not counted once as RSA and again as generic
HRV. A seed draws one subject heart-rate phenotype; changing state moves that same subject around
stage-specific group means rather than drawing a new person.

**Empirical anchor.** A causal QRS detector applied identically to ECG from 19 locally cached HMC
nights gives stage medians for heart rate and SDNN: wake 68.3 bpm/117.5 ms, N1 63.1/107.0, N2
61.6/76.7, N3 62.1/61.3, and REM 65.1/81.8. Those are fitted targets, with a shared 0.21
between-subject heart-rate CV measured across the same records. HMC has no respiratory channel,
so it cannot identify RSA. Penzel et al.'s normalized stage ratios instead constrain the relative
RSA ordering: wake 0.90, light NREM 1.33, deep NREM 1.37, REM 1.00. The absolute REM amplitude,
phase offset and latent variance allocation remain pending rather than being promoted by the
generated output.

**Continuity consequence.** The live stream owns `CardiacState` beside `RespiratoryState`; export
and display advance the same transitions. The PQRST renderer is causal at the R boundary: waves
before R use the known preceding interval and waves after R use the newly scheduled interval.
This clips only the negligible four-sigma Gaussian tail on the opposite side of R, and prevents a
future interval that is unavailable at a chunk boundary from changing already rendered samples.
Arbitrary split and JSON-resume tests therefore reproduce ECG samples and R-peak times exactly.

**Acceptance result.** Across 20 seeds × 1,200 seconds per state, generated median heart rate,
SDNN and RMSSD all lie inside the corresponding HMC interquartile intervals. Recovered respiratory
RR amplitudes are 26 ms in wake, 39 ms in N1, 38 ms in N2, 39 ms in N3 and 29 ms in REM, close to
the requested 27/40/40/41/30 ms. In the browser's actual 90-second prefetch-and-roll path, ECG sample
error and spanning-RR error are both exactly zero. Generator version 0.7.0 records the output
change.

**Claim boundary.** R2 validates timing behavior and display continuity. The PQRST form still
comes from the registered McSharry procedure, but its wave amplitudes and widths have not been fit
against a real morphology corpus. The ECG is not a diagnostic ECG simulator.

---

## D23 — respiratory EEG coupling has separate spatial, aperiodic and periodic causes

**Decision.** The analytic respiratory phase is peak-inspiration-centred: inspiration occupies
`-pi..0`, expiration `0..pi`, and zero lies at the centre of the realized peak-inspiration
plateau. Aperiodic slope modulation uses the five state-specific maximum-slope directions reported
by Sánchez Corzo et al.: 2.99 rad in wake, -2.70 in N1, -1.33 in N2, -1.78 in N3 and -1.45 in REM.
These replace the former shared wake/sleep phase switch.

**Spatial decision.** A named posterior cortical patch—pericalcarine, cuneus, lingual,
lateral-occipital, superior-parietal and precuneus—enters the fsaverage three-shell BEM solely as
a modulation loading. The root-sum-square across its signed spatial modes gives a nonnegative
power-effect loading normalized at the scalp maximum. This produces a posterior maximum and a
physical frontal tail without adding a hand-drawn electrode map or another signal source.

**Composition boundary.** The loading modulates the already mixed distributed and channel-local
aperiodic background. Each channel is then returned to its pre-modulation RMS, so respiratory
slope coupling cannot silently alter the accepted background amplitude or SNR. Realized breath
depth scales the coupling; the independent G4 fixture remains a pure single-frequency modulation
so its falsification arm does not acquire respiratory sidebands.

**Periodic decision.** Periodic and aperiodic effects do not share phase. Low-frequency periodic
amplitude peaks at the beginning/end of the peak-inspiration-centred cycle; high-frequency
amplitude uses the reversed course. Their gain is
`exp(m cos(phi - phi0)) / sqrt(I0(2m))`, which is positive and has unit mean square over a breath.
This replaces the linear `1 + d cos(phi)` multiplier that changed mean power and could become
negative. Applying the wake-derived periodic phase form to sleep is explicitly provisional.

**Acceptance result.** In matched mechanism-off records spanning all six states, recovered
aperiodic maximum phase is within 1–4 degrees of the injected direction. The recovered 19-channel
depth map correlates 0.999 with the independently computed BEM loading, and recovered Fz depth is
26–33% of Pz rather than zero. Mean rhythm amplitude and total EEG RMS remain at 1.000 of the
matched off record (maximum total-RMS deviation 0.02%). Periodic maximum phase is recovered within
0–10 degrees; its observed depth appropriately depends on whether the measured rhythm is actually
prominent at the selected electrode.

**Claim boundary.** The phase directions and spatial construction are anchored; the absolute
`chi_mod_depth`, low-band `resp_amp_mod_depth` and high-band `periodic_mod_depth_high` are not.
Generated recovery cannot promote them. Generator 0.8.0 and export schema 3 record the changed
signal and truth contract.

---

## D24 — respiratory event coupling moves event times, never event counts

**Decision.** R4 draws the baseline Poisson event count exactly once. Coupling redistributes the
resulting event markers over respiratory phase but cannot add or remove an event. SO downstates
use Schreiner et al.'s -9.4-degree mean and 0.51 resultant length; fast-spindle onsets use +16.95
degrees and 0.52. The corresponding von Mises concentrations are derived by solving
`I1(kappa)/I0(kappa) = R`, not fitted against generated recovery.

**Phase-exposure correction.** Respiratory phase is not uniform in clock time because inspiration,
expiration and pauses differ in duration. A clock-time hazard recovered the requested
concentration but shifted the natural-breath mean by 0.19–0.31 rad. The accepted sampler draws
the target phase first and then chooses among record samples carrying that phase. Thus respiratory
morphology determines when a phase occurs without becoming a hidden preferred-phase parameter.
A raw shuffled/null phase histogram retains the I:E exposure asymmetry; the paired mechanism-off
record, not an assertion of circular flatness, is the valid null.

**Marker semantics.** The slow-oscillation source value refers to its negative downstate, which
occurs at `so_rdsym/2` of the negative-first waveform—not at the array boundary. The spindle value
refers to onset. Event truth records both the marker offset and its realized respiratory phase.
This distinction is why export schema 4 is required.

**Joint-coupling correction.** The old N3 scheduler described a spindle as up-state coupled while
adding its phase to the slow-wave array start. R4 locates the waveform's actual positive maximum,
subtracts half the spindle duration to express an onset, and combines that candidate process with
the respiratory marginal. Only fast spindles take this path. Leaving slow spindles nested inside
respiration-coupled SOs would create indirect respiratory locking while claiming they were a
negative control.

**Acceptance result.** Five seeds × one hour per state preserve every injected count exactly.
Slow-spindle onset, duration and amplitude are bit-identical across the respiratory on/off arms.
Natural-breath injected truth recovers SO phase -0.169 rad (target -0.164, R=0.517), N2 fast
spindles 0.344 rad (target 0.296, R=0.498), and N3 fast spindles 0.182 rad (target 0.296,
R=0.512). YASA applied to the complete mixture recovers SO-before-inspiration and fast-spindle-
after-inspiration ordering. Its N3 spindle onset is delayed relative to injected onset, which is
recorded as detector bias rather than used to retune the source.

**Competing evidence boundary.** Schreiner et al. found no significant slow-spindle concentration
in their nap data. Girin et al. 2024 subsequently reported both slow and fast spindles concentrated
during expiration in full-night PSG, plus biphasic slow-wave timing. The shipped profile is the
pre-specified Schreiner profile, not a universal claim. `resp_spindle_slow_hazard` is registered
as absent and names the conflict; changing profiles requires an explicit source/corpus decision.
Generator 0.9.0 records the timing change.
...
| ~~P9~~ | ~~Replace Gaussian projection weights with a lead field~~ | — | **Closed, shipped (D19, D19.1, Findings 19–21).** fsaverage 3-shell BEM over Desikan–Killiany patches, spatial eigenmodes per patch. 31 invented rows deleted, 4 added. wake_ec effective rank 5.43 vs a real 5.36 under average reference, mean relative error 0.107 against 0.250 for the mixture it replaced |
...
## D25 — regular respiration is a causal contrast, not a realism setting

**Decision.** The released UI offers `natural` and `regular` respiration. Natural uses the
state-dependent subject phenotype, period/depth variability, correlated timing drift, variable
I:E ratio and stochastic pauses established in R1. Regular uses the same state's registered
population mean rate but fixes period, depth, duty cycle and pauses. It is explicitly a teaching
contrast and is not described as a second physiological phenotype.

Movement artifact has its own switch. That switch removes only the mechanical electrode term;
it does not disable periodic neural gain, aperiodic slope modulation, NREM event timing or RSA.
There is no global respiration-strength control because those mechanisms have different causal
origins and scalp consequences.

**Truth contract.** Epoch schema v5 records controller truth rather than estimates recovered from
generated EEG: realized breaths, variability and morphology summaries; R peaks, RR intervals,
HRV and a harmonic RSA summary against known respiratory phase; and circular event-marker
summaries. Variable-length arrays live once in run-level `physiology.json`; each epoch sidecar
keeps the compact summaries and names that file. This prevents an eight-hour record from copying
the same thousands of events into hundreds of sidecars. Values unavailable in a short record are
`null`. This changes the export schema but not the default generated signal, so
`generator_version` remains 0.9.0.

**Circularity constraint.** The R5 interaction audit may assert exact structural invariants and
record marginal signal changes. It cannot promote a pending coupling magnitude merely because a
measurement of our own output recovers it. Finding 41.

---

## D26 — infra-slow voltage, excitability and drift are different causes

**Decision.** Activity below 0.1 Hz enters through three separately switchable paths: an additive
cortical-current component projected through the existing BEM, a latent infra-slow state that
modulates faster cortical source amplitudes with unit mean-square gain, and an optional
electrode/skin/amplifier drift component outside the cortical forward model. Respiration remains
its own fourth cause. No path may borrow another's amplitude merely because all can occupy the
same frequency range.

**Reason.** General EEG simulators can project any slow source through a lead field, and
biophysical network models can generate infra-slow dynamics, but neither fact establishes that
the complete full-band scalp potential is a cortical current dipole. Healthy full-band studies
report BBB, vascular and other physiological interpretations alongside neural coupling. Forcing
all observed DC voltage through the BEM would turn a useful physical model into an unsupported
origin claim.

**Spatial decision.** The neural component begins with a small number of broad named cortical
patch families represented by the lead field's existing covariance modes. Shared state and delay
live in source space. No channel receives a hand-authored slow amplitude or phase. Evidence for
network-specific full-band components rules out one global waveform as the default, but does not
yet supply a shared-source fraction or delay distribution.

**State boundary.** ISF-0 accepts only the externally observed aggregate contrast NREM > wake for
infra-slow power and coupling extent. It does not manufacture an N1/N2/N3 ladder, and REM remains
absent. Published PLV values are estimator-specific reference metrics, not generator modulation
depths.

**Prefix.** ISF-0 changes no generated sample and therefore does not bump `generator_version`.
Unknown cortical RMS, source coherence, delay, modulation depth, REM profile and electrode-drift
amplitude are registered as `absent`, not provisional. The current aperiodic background's
incidental sub-0.1 Hz power must be measured in ISF-1 before a named additive component is added,
or the implementation would double-count a spectrum it had not characterized.

**Execution contract.** `docs/Infraslow-Source-Projection-Plan.md` defines I0-I7 and ISF-0 through
ISF-6. Every later coupling gate uses matched mechanism-off arms, and no tolerance or magnitude is
derived from the simulator's own spread.

---

## D27 — infra-slow time is a band-limited power-law state, not a bank of named rhythms

**Decision.** The temporal family selected at ISF-1 is a continuous band-limited power-law
state-space controller with separate ISF1 and ISF2 outputs. It runs at a chosen 2 Hz controller
rate and later interpolates to EEG sampling rate. Its spectral exponent, approximation order,
state gains and voltage amplitude remain absent.

**Comparison.** Twenty normalized 1,250-second realizations of three model families were measured
outside `composeState`: power-law state space, a damped-oscillator bank and a power-law/weak-
resonance hybrid. All three were sample-identical under arbitrary chunking and produced broad,
irregular periods. Their ISF1 largest-bin shares were 0.076, 0.082 and 0.080; normalized spectral
entropies 0.838, 0.834 and 0.838. No temporal realism difference justified the oscillator bank's
centre/damping parameters or the hybrid's resonance location and mixture fraction.

**The 0.02 Hz trap.** The hybrid's median peak landed at 0.02 Hz, which the external study describes
as commonly observed. It was not selected. A common peak is not evidence that every record has a
fixed resonant component, and the source supplies no mixture weight. `isf_resonance_fraction` is
therefore absent rather than set from the candidate whose spectrum we manufactured.

**Existing-spectrum boundary.** Before adding a named source, the current continuous generator was
measured over the full I0 record. It already produces 0.49-4.22 uV RMS below 0.1 Hz under linked
mastoids, 9.7-14.3% of its 0.1-1 Hz power depending on state. This is incidental content from the
existing aperiodic and rhythm sources, not validation of an ISF mechanism. It is also not zero.
ISF-2/3 must account for it rather than adding a new target amplitude on top and then fitting the
sum as though the incumbent spectrum did not exist.

**Circularity constraint.** Candidate realizations were normalized and compared only for form,
memory and continuity. Their fixture exponent, pole count and weak-resonance fraction do not enter
the registry as values and cannot reach the shipped generator. No amplitude or gate threshold was
derived from them. Finding 43.

---

## D28 — three anatomical systems are the infra-slow spatial prefix

**Decision.** The cortical infra-slow basis contains three bilateral Desikan-Killiany patch
families: frontomedial association, sensorimotor and posterior visual/association cortex. Each is
projected through the existing fsaverage three-shell BEM and represented by enough covariance
eigenmodes to satisfy `patch_mode_variance`. A proposed lateral-association family remains absent.

**Why three.** Full-band components are associated with subsets of resting-state networks, which
rejects one global waveform, while the healthy wake/NREM anchor reports broadly distributed power
with frontal enhancement. An anterior/central/posterior basis is the smallest anatomical prefix
that represents those spatially distinct systems. No independent observation currently requires
a fourth family. Adding one to improve effective rank, PC1 variance or correlation would violate
the stop rule because all three are summaries of the same covariance matrix.

**Forward-model boundary.** Every emitted `isf_*` mode has named atlas regions and provenance
`leadfield_patch_eigenmode`. No infra-slow electrode weights are authored outside the projection
producer, and no channel carries an assigned phase or delay. Referencing is the ordinary linear
operator applied after projection; propagation, if later supported, must be implemented as a
source-space delay.

**Characterization, not fit.** Independent unit-variance mode drivers give linked-mastoid peaks at
Fz, C3 and Pz. The equal-family covariance has effective rank 3.480, PC1 fraction 0.410 and median
absolute channel correlation 0.313. These are predictions of a deliberately amplitude-free
fixture, not three fit targets and not evidence that the physiological family variances are equal.

**Release boundary.** The spatial bases are present but undriven. Cortical RMS, shared fraction,
source delay, state gain and PAC depth remain absent; generated samples and truth are unchanged,
so neither generator nor export schema version moves. Finding 44.

---

## D29 — infra-slow coupling changes source gain, not accepted carrier power

**Decision.** The ISF excitability path is a positive log-amplitude gain applied to named cortical
sources before projection:

```text
g(z) = exp(m z - m²),  z ~ N(0,1)
```

Therefore `E[g²] = 1` analytically. The additive BEM voltage and source-gain paths have independent
switches and matched off, additive-only, modulation-only, both and π-inverted-driver arms. The
driver inversion supplies a phase null without shifting or regenerating the carrier.

**No defaults from fixtures.** The executable path is `infraSlowFixture`, and its name is literal:
the caller must provide the temporal exponent, approximation order, ISF1 variance share, additive
RMS, target mapping and modulation depth. None has a fallback. This lets I4/I5 and future external
fits exercise the actual architecture without allowing local characterization values to become
the shipped physiology.

**Source boundary.** Additive activity drives every covariance mode of a selected ISF family and
uses only the BEM projection seam. Modulation acts on a named continuous source before its channel
projection. The target map remains absent because electrode-level coupling extent does not identify
which of this simulator's cortical rhythm patches was modulated.

**Streaming and variance.** The causal two-band controller runs at 2 Hz, is JSON-serializable and
is sample-identical under arbitrary chunking. Its stationary scale is derived from impulse energy
of the accepted linear state-space system. It is not normalized against a generated record. A
finite record's realized gain RMS is still reported because ensemble power preservation does not
mean every short realization has RMS exactly one.

**Respiration boundary.** ISF and respiration use independent controllers and substreams. In the
matched separation test, changing respiration from 10 to 20 breaths/min changes the belt while
leaving all EEG samples bit-identical when respiratory EEG mechanisms and event timing are off.
An unexplained shared physiological driver remains forbidden.

**Release boundary.** Default composition does not request the fixture and therefore emits neither
ISF samples nor ISF truth. Generator and export schema versions remain unchanged. Finding 45.

---

## D30 — full-band scalp amplitude is not cortical-source amplitude

**Decision.** ISF-4 is held at the estimand boundary. No available public record is eligible to
fit `isf_cortical_rms_*`, and the fixture values do not become defaults. The external audit is
executable in `prep/reference/isf4_external_evidence.mts`; the registry test prevents a later
total-scalp fit from silently promoting the source rows.

**Evidence audit.** The primary healthy DC-coupled wake/NREM study publishes relative power and
estimator-specific PAC, not absolute source amplitude, and its raw records are not public. Two
public healthy wake corpora are DC-capable but their 3- or 5-minute continuous runs do not span the
registered ten cycles at 0.008 Hz. A public scored W/N1/N2/N3 EEG-fMRI corpus has a nominal 0-250
Hz passband and 15-minute runs, but the raw voltage contains MRI gradient and BCG artifacts; its
artifact-corrected staging stream was filtered at 0.3-35 Hz. None supplies a REM anchor.

**Identifiability.** Even an artifact-free DC scalp record observes the sum of cortical current,
BBB/vascular physiology, respiration, electrode/skin polarization and reference behavior. A
three-shell BEM models only the cortical-current term. Matching that term to total observed RMS
would therefore be a causal attribution disguised as amplitude calibration.

**What remains useful.** Published NREM > wake relative power, PAC PLV and coupled-channel extent
remain external comparators. The public wake and staged-sleep corpora can later characterize
restricted upper-infraslow bands, artifact methods and state directions, but cannot identify the
released additive cortical RMS or source-level modulation depth. Finding 46.

---

## D31 — recording drift is an electrode-space observation, not a weak cortical source

**Decision.** ISF-5 adds a fixture-only recording path after cortical composition. Independent
electrode drift is added directly to every generated lead. A separate shared mastoid drift is
added only to A1/A2, allowing the existing linked-mastoid and contralateral operators to propagate
reference-electrode contamination into scalp derivations. Neither term has a default amplitude.

**Why this prefix.** DC stability depends strongly on electrode material, gel, skin preparation,
perspiration, contact and amplifier. Intact-skin recordings can show unpredictable channel drift,
while suitable Ag/AgCl systems can be orders of magnitude more stable. A universal amplitude,
state profile or waveform would therefore describe an unspecified recording system. The broad
causal stochastic controller is reused as fixture form only; fitting awaits a named hardware and
preparation corpus.

**Reference consequence.** A shared mastoid term changes linked-mastoid and contralateral
derivations exactly, but leaves as-generated scalp, average reference and Laplacian bit-identical.
That is not a hand-authored scalp topography: it is the output of the selected reference operator.
Independent lead terms remain spatially high-rank and do not obey the cortical BEM.

**Gate boundary.** Drift truth contains no cortical source modes and no modulation targets, so it
cannot enter I2-I5 as a neural positive arm. A high-pass can remove both cortical direct voltage
and electrode drift; truth, not filter response, preserves the causal distinction. Fixture
measurements characterize this separation but set no amplitude or acceptance tolerance.

**Release boundary.** `electrode_dc_drift_rms` and the newly separated
`reference_dc_drift_rms` remain absent. The fixture is not requested by the UI or exporter;
default samples and truth are unchanged, so generator 0.9.0 and epoch schema v5 do not move.
Finding 47.

---

## D32 — the full-band view is continuous and causal controls fail closed

**Decision.** ISF-6 adds a lazy 120/300/600-second full-band overview beside, rather than inside,
the clinical scroll. Each overview is one continuous `composeState` record under the current
seed, state, respiratory settings and reference. It is never assembled from the scroll's
90-second presentation buffers, whose joins are crossfaded and explicitly ineligible for
long-timescale analysis.

**Two views, two jobs.** The clinical trace remains a fixed-scale 5-60-second paper scroll. The
full-band trace anti-aliases both arms identically to 2 samples/s for display and compares a
DC-retaining arm directly with a fixed 0.1 Hz zero-phase high-pass. Its spectrum uses a
Hann-windowed complete-record periodogram rather than the live panel's four-second Welch
segments and draws the 0.005-1 Hz range. The interface states `1 / duration` because FFT zero
padding makes a curve denser but cannot manufacture additional cycles.

**Fail-closed causal truth.** Projected cortical voltage, source-level excitability modulation
and electrode/reference drift appear as three independent causal layers, but their controls are
disabled while the corresponding amplitude/depth rows remain `absent`. Their fixture paths and
matched nulls are executable; the release UI may not choose a visually satisfying fixture and
call it physiology. The released overview consequently contains the generator's existing
incidental sub-0.1 Hz aperiodic content and respiratory effects, but no named infra-slow truth.

**Interpretation.** A high-pass distinguishes frequencies, not causes. Removing a visible slow
potential does not establish cortical, respiratory, vascular, skin/electrode or reference
origin. That statement remains visible beside the comparison rather than being left to
documentation.

**Release consequence.** These are registry-backed display and estimator choices only. Default
generated samples, export fields and truth are unchanged, so generator 0.9.0 and epoch schema v5
do not move. Finding 48.

---

## D33 — estimated cortical ISF is released as a power-budgeted, source-level model

**Decision.** The ISF-6 fail-closed boundary was too categorical for a simulator. Evidence does
not identify the cortical fraction of total scalp DC voltage, but it does support continuous
healthy infra-slow activity, aggregate NREM > wake power with frontal emphasis, and coupling of
ISF1 phase to fast-rhythm amplitudes. Generator 0.10.0 therefore releases the existing causal
mechanisms with conspicuous `pending` registry values rather than presenting uncertainty as
absence. Recording drift remains fixture-only and deferred.

**Amplitude and power budget.** Wake receives a provisional 2 µV aggregate cortical source
budget, aggregate NREM 3 µV, and REM a wake-derived 2 µV extrapolation. The released cortical
budget is subtracted in quadrature from the existing aperiodic background allocation before the
named source is added. It therefore partitions an existing variance budget instead of stacking a
new slow waveform on top of a previously fitted signal. Wake uses the three anterior, central and
posterior BEM families; NREM uses the frontomedial family as the smallest prefix consistent with
the published frontal power emphasis. No N1-N3 amplitude ladder is introduced.

**Coupling map.** A single global gain failed the scalp calibration because visible ISF voltage
was a mixture of many BEM modes while every carrier mode followed one controller. Each carrier
mode now follows the corresponding mode of its anatomical ISF family. Wake maps posterior alpha
and sensorimotor beta; aggregate NREM maps represented canonical rhythms to the frontomedial
controller. Distributed aperiodic modes, channel-local covariance residuals and discrete events
are not modulated. Modulating the aperiodic background was rejected because it allowed that
background alone to satisfy the AASM slow-wave criterion after named slow-wave sources were
attenuated. The controller remains continuous across the live scroll's 90-second buffer changes.

**External comparator.** On one 1250-second linked-mastoid record, a Väyrynen-structured
ISF1/fast-envelope estimator gives wake-EC mean PLV 0.169 against the published wake 0.178, and N2
0.205 against aggregate sleep 0.211. N2 has 7/19 significant channels (0.368), one electrode above
the reported sleep interval 0.05-0.35 at 19-channel granularity. Wake has 4/19 (0.211), above the
high-density wake interval 0.05-0.15; these disagreements remain visible and are not tuned away
on one seed or a 19-channel montage. Median 0.008-0.1 Hz RMS is 1.24 µV in wake-EC and 2.39 µV in
N2; frontal N2 median is 3.06 µV,
preserving both external state directions. The estimator differs from the paper's long-FIR
implementation, so these are record-only comparators, not pass thresholds.

**Release consequence.** Named ISF truth now appears by default and records whether it came from
the provisional release or an explicit fixture; REM truth is marked extrapolated. Complete
source-mode truth is written once to `physiology.json`, with a compact index in each epoch
sidecar. The generator moves to 0.10.0 and the export truth schema to v6. See D35 for the
subsequent calibration and interface corrections; the previously cited Finding 49 had not
been written at this point.

---

## D34 — the high-pass control extends below the mechanism it demonstrates

**Decision.** `filter_ui_range.lo` and `hpf_default` are 0.005 Hz, below the named 0.008–0.1 Hz
infra-slow band and the existing 0.01 Hz high-pass option. Raising the handle and switching the
filter off remain different operations.

**Reason.** The previous UI floor was 0.1 Hz even though its registry note claimed it sat below
0.01 Hz. That made the ordinary filter panel incapable of retaining the direct cortical voltage
it was meant to demonstrate. Initially leaving the opening cutoff at 0.1 Hz preserved the same
failure by default: the fourth-order zero-phase response has effectively zero gain at 0.008 and
0.02 Hz, 0.0039 at 0.05 Hz and 0.1437 at 0.08 Hz. At 0.005 Hz it retains 0.9772 at the lower ISF
edge. The opening trace therefore now preserves the mechanism, while raising the handle remains
the demonstration of its loss. A short visible window still cannot resolve a 0.005 Hz response;
selectable cutoff and estimator resolution are not the same claim.

**Spectrum consequence.** A complete-record periodogram was tried and rejected: it moved the first
Fourier bin to about 0.011 Hz, but its variance does not converge and the resulting teeth were
estimator noise, not credible physiology. The live panel now uses four or more 50%-overlapped,
long-window Welch segments below 1 Hz, followed by a five-bin triangular frequency average. At
the complete 90-second buffer this means 32-second windows and 0.03125 Hz resolution. The estimate
blends into ordinary four-second Welch from 0.5 to 1 Hz and uses ordinary Welch alone above 1 Hz.

Frequencies below the long-window resolution are shaded and labelled unresolved. The analytic
filter response continues through that region because coefficients have a response there; the
signal PSD does not pretend to estimate it. The spectrum source is always the unfiltered referenced
channel, independent of whether the optional raw trace overlay is visible, preventing the former
filtered-twice path. The three display-estimator choices are registered separately from every
generator parameter.

---

## D35 — release configuration, scoring and integration are explicit contracts

The browser and exporter now share `physiology-v1`. Calibration uses that configuration with
neutral infra-slow gain, retains full precision, and replays the serialized result. A changed
model fingerprint requires recalibration. G4 selects an isolated fixture explicitly; ordinary
exports no longer silently disable respiratory mechanisms or fall back to zero SNR.

The operational C3-A2 scorer is specified in `Scoring-Contract.md`: cascaded Butterworth high/low
sections, reflected padding, steady-state forward/reverse filtering, and shared half-wave
boundaries. TypeScript and independent SciPy implementations are tested on identical samples.
Agreement between them establishes this numerical contract, not expert-scoring equivalence.

The arbitrary prominence draw is retained as `inclusionTag`, with its quality interpretation
removed. The continuous full-band view is exposed separately from presentation-corrected clinical
segments. Epoch schema moves to 7, event-list schema to 2, and generator/package to 0.11.0.

Five additional HMC subjects were named before download and analysis. Their results are retained
without fitting parameters or acceptance thresholds to them. The corrected development analysis
and the released-strength coupling probe also remain visible when they disagree with intended
behavior. See `Stabilization-0.11.0.md` and Finding 49 for measured outcomes and remaining work.
